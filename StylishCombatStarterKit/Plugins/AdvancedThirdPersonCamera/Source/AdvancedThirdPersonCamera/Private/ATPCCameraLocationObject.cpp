...
void UATPCCameraLocationObject::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
    ...
    PitchCameraDistanceModifier = locationSettings.PitchDistanceCurve != nullptr
        ? locationSettings.PitchDistanceCurve->GetFloatValue(FRotator::NormalizeAxis(desiredRot.Pitch))
        : 0.f;

    auto& cameraMode = GetCamera().GetCurrentCameraMode()->CameraModeSettings;
    const float targetCameraDistance = GetTotalCameraDistance();
    const float newCameraDistance = UATPCFunctionLibrary::InterpFloat(
        CameraDistance.CurrentDistance,
        targetCameraDistance,
        DeltaTime,
        cameraMode.LocationSettings.ZoomInterpolation,
        false);
    ...
    if (bDoTrace)
    {
        ...
        collisionQueryParams.bTraceComplex = true; // Ensure complex collision is considered
        ...
        GetWorld()->SweepSingleByChannel(
            ...
            traceEndOldDist,
            FQuat::Identity,
            locationSettings.ProbeChannel,
            FCollisionShape::MakeSphere(locationSettings.ProbeSize),
        ...
        // If some modifiers cause collision, use modifiers from prev time (if it wasn't collision)
    ...
    // Check if the camera's current distance exceeds the maximum allowed distance
    const float currentCamDistance = FVector::Distance(armOriginWithLag, resultLoc);
    if (currentCamDistance > locationSettings.MaxCameraDistance)
    {
        // Interpolate the camera distance towards the default camera distance
        const float defaultCameraDistance = locationSettings.DefaultCameraDistance;
        const float interpSpeed = locationSettings.CameraResetInterpolationSpeed; // Define this speed in your settings
    	float interpolatedCameraDistance = FMath::FInterpTo(
			CameraDistance.CurrentDistance,
			defaultCameraDistance,
			DeltaTime,
			interpSpeed);

        InternalSetCurrentCameraDistance(interpolatedCameraDistance);

        // Recalculate desired location with interpolated camera distance
        FVector interpolatedDesiredLoc = calcDesiredLocation(interpolatedCameraDistance, GetAllSocketOffsets());

        // Smoothly interpolate the result location
    	resultLoc = BlendLocations(resultLoc, interpolatedDesiredLoc, false, FVector::Zero(), DeltaTime);

        // Set bIsCameraFixed to false
        bIsCameraFixed = false;

        // Debugging information
        UE_LOG(LogTemp, Warning, TEXT("Camera distance exceeded max distance. Interpolating camera position to default distance."));
    }
    // NEW CODE ENDS HERE
